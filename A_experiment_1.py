...
def train_rl_agent(scheme_name, agent_class, use_masking, use_embedding, sumo_cfg_path, 
                   pretrained_encoder, pretrained_predictor, episodes, seed):
    config = V2XConfig()
...
def run_experiment_1():
    sumo_cfg_path = "sumo_jtr_scenarios/turn_.sumocfg"
    ...
    print("="*60)
    set_seed(42)
    encoder, predictor = pretrain_intention_encoder(sumo_cfg_path, num_steps=5000, seed=42)
    ...
    for seed in SEEDS:
        ...
        r_std, f_std, c_std, v_std = train_rl_agent(
            ...
            episodes=TOTAL_EPISODES, seed=seed
        )
    ...
    for key in results.keys():
        ...
        # 🌟 [심층 진단] 평균 유효 액션(마스킹 후 남은 차량) 수 분석
